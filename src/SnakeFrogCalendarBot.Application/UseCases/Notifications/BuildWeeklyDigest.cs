...
public sealed class BuildWeeklyDigest
{
    ...
    public async Task<WeeklyDigestResult> ExecuteAsync(CancellationToken cancellationToken)
    {
        ...
        return new WeeklyDigestResult(periodStart.Date, periodEnd.Date, items, timeZone.Id);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var eventEntity in allEvents)
        {
            ...
            else if (eventEntity.Kind == EventKind.Yearly && eventEntity.Month.HasValue && eventEntity.Day.HasValue)
            {
                ...
                if (targetDate < periodStart.Date)
                {
                    targetDate = new LocalDate(periodStart.Year + 1, eventEntity.Month.Value, eventEntity.Day.Value);
                }
        ...
        foreach (var birthday in allBirthdays)
        {
            ...
            if (birthdayDate < periodStart.Date)
            {
                birthdayDate = new LocalDate(periodStart.Year + 1, birthday.Month, birthday.Day);
            }
