...
Проект уже не на стадии "плана". `AGENTS.md` должен отражать текущее состояние кода, а не желаемую архитектуру из будущих спринтов.
...
- `Notifications`: build daily/weekly/monthly digest + send digest
...
`AccessGuard` сейчас находится в `Worker/Telegram`, а не в Infrastructure.
...
- расчеты периодов дайджестов: `tests/SnakeFrogCalendarBot.Tests/Notifications`

При изменении логики парсинга, TZ или расчета периодов тесты нужно обновлять вместе с кодом.
...
4. Любые изменения парсинга даты или периодов дайджеста сопровождать тестами.
